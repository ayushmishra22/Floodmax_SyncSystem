...
public class FileIO {
	...
	int[] breakStringIntoArray (String s, int numProcesses, int[] UIDs){
		...
		for(int elem = 0; elem < splitString.length; elem++) {
			// for each element in the array we need to set the value to 1
			parsedArray[Integer.parseInt(splitString[elem]) - 1] = 1;
		}
		/*
				...
				while(reader.hasNextLine()) {
					...
					switch(fileLine) {
						case 0: // This is used to determine the number of processes there are in the program
							...
							UID = new int[numberOfProcesses];		// Now that we now the number of processes, we can set the number of UIDs to expect
							adjInformation = new int[numberOfProcesses][numberOfProcesses]; // same as above ^
							break;	
						case 1: // This is used to determine the UIDs of the processes 
							UID = breakStringIntoArray(data, numberOfProcesses);
