...
public class FileIO {
	...
	int[] breakStringIntoArray (String s){
		// A way to read in a string and parse the information so that it can be applied to an array 				// An array that holds the integers
		String[] splitString = s.split(" ");		// Split by the string
		int []parsedArray = new int[splitString.length];
	...
	int[] breakStringIntoArray (String s, int numProcesses, int[] UIDs){
		...
		for(int elemInNeighbors = 0; elemInNeighbors < splitString.length; elemInNeighbors++) {
			...
			// Go to each position of the smaller string and set the value to 1 if the neighbors are true
				if(elemInUID + 1 == Integer.parseInt(splitString[elemInNeighbors])) {
				...
				while(reader.hasNextLine()) {
					...
					switch(fileLine) {
						case 0: // This is used to determine the number of processes there are in the program
							...
							adjInformation = new int[numberOfProcesses][];
							break;	
